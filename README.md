...
`#` prefix marking an encrypted value. Declare the mapping with `#[RequestKeyMap]` on the payload
class, so the wire contract lives next to the properties it describes:
...
directly under a rename target (`data` here) is ignored — only the declared wire key may populate
